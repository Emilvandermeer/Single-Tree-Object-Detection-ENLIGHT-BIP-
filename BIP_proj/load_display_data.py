...
def load_files():
    ...
    def display_data():
        ...
        CHM_img_path = "training/CHM/2018_SJER_3_258000_4106000_image_CHM.tif"
        HS_img_path = "training/Hyperspectral/2018_BART_4_322000_4882000_image_crop_hyperspectral.tif"
        LiDAR_img_path = "training/LiDAR/2018_BART_4_322000_4882000_image_crop.laz"
        RGB_img_path = "training/RGB/2018_BART_4_322000_4882000_image_crop.tif"

        tif_imgs = [("CHM",CHM_img_path),("HS",HS_img_path),
                    ("RGB",RGB_img_path)]

        def show_tif(images:list[str,str])->None:
            """
            Function to show the tif files
            Input: list(name,filepath)
            Output: None
            """
            for name, path in images:
                ...
                if name == "CHM":
                    ...
                    img = plt.imshow(img, cmap='viridis', interpolation='nearest')
                    plt.colorbar(img, label='Canopy Height (m)')
                    ...
                    fig.suptitle(f"{name} Image - 3 Bands", fontsize=16)
                    for i in range(3):
                        band = img[:,:,i]
                        ax[i].imshow(band)
                    plt.show()
